                    ...
                    for (ExecutorSaveStateInstance instance : mementoList) {
                        if (instance instanceof  ExecutorSaveStateInstance.Execute) {
                            ...
                            if (!scheduleDelay.task.isCancelled()) {
                                scheduledExecutorService.schedule(scheduleDelay.task, scheduleDelay.delay, TimeUnit.MILLISECONDS);
                            }
                            ...
                            if (!scheduleDate.task.isCancelled()) {
                                scheduledExecutorService.schedule(scheduleDate.task, scheduleDate.time.getTime(), TimeUnit.MILLISECONDS);
                            }
